      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 bg-[#f5f6f8] flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-scroll">
            {activeTab === "analytics" ? (
              <AnalyticsTab slug={slug} linkLabels={{...Object.fromEntries(links.filter(l => l.layout !== "kakaotalk").map((l) => [l.id, l.label])), "kakaotalk-chat": "카톡 문의"}} linkUrls={{...Object.fromEntries(links.filter(l => l.layout !== "kakaotalk").map((l) => [l.id, l.url])), "kakaotalk-chat": links.find(l => l.layout === "kakaotalk")?.url || ""}} socialUrls={Object.fromEntries(socials.map((s) => [s.platform, s.url]))} />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {openProfile && (
                <div className="px-5 pb-5 flex flex-col gap-3 border-t border-gray-50 pt-4">
                  ...
                  <div><label className="text-xs font-medium text-gray-500 mb-1 block">상세문구</label><textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="추가 설명 (Enter로 줄바꿈)" rows={3} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 resize-none" /></div>
